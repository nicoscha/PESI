...
class Test_ESIReader(unittest.TestCase):
    ...
    def test_get_parameters_with_description(self):  # TODO lines to long
        ...
        parameters = {'Accept-Language': {'default': 'en-us',
                                          ...
                                          'type': 'string'},
                      'If-None-Match': {'description': 'ETag from a previous request. A 304 will be returned if this matches the current ETag',
                                        'in': 'header', 'name': 'If-None-Match',
                                        'type': 'string'},
                      'alliance_id': {'description': 'An EVE alliance ID',
                                      'format': 'int32', 'in': 'path',
                                      'minimum': 1, 'name': 'alliance_id',
                                      'required': True, 'type': 'integer'},
                      'character_id': {'description': 'An EVE character ID',
                                       'format': 'int32', 'in': 'path',
                                       'minimum': 1, 'name': 'character_id',
                                       'required': True, 'type': 'integer'},
                      'corporation_id': {'description': 'An EVE corporation ID',
                                         'format': 'int32', 'in': 'path',
                                         'minimum': 1, 'name': 'corporation_id',
                                         'required': True, 'type': 'integer'},
                      ...
                      'page': {'default': 1,
                               ...
                               'format': 'int32', 'in': 'query', 'minimum': 1,
                               'name': 'page', 'type': 'integer'},
                      'token': {'description': 'Access token to use if unable to set a header',
                                'in': 'query', 'name': 'token',
                                'type': 'string'},
                      'accept_language': {'default': 'en-us',
                                          'description': 'Language to use in the response',
                                          'enum': ['de', 'en-us', 'fr',
                                                   'ja', 'ru', 'zh'],
    ...
    def test_create_doc_string(self): # TODO lines to long
        ESI_parameters = {'Accept-Language':
                              {'default': 'en-us',
                               'description': 'Language to use in the response',
                               'enum': ['de', 'en-us', 'fr', 'ja', 'ru', 'zh'],
                               'in': 'header', 'name': 'Accept-Language',
                               'type': 'string'},
                          'If-None-Match': {'description': 'ETag from a previous request. A 304 will be returned if this matches the current ETag',
                                            'in': 'header',
        ...
        function_dict = {'description': 'Return a list of kills related to a war\n\n---\nAlternate route: `/dev/wars/{war_id}/killmails/`\n\nAlternate route: `/legacy/wars/{war_id}/killmails/`\n\nAlternate route: `/v1/wars/{war_id}/killmails/`\n\n---\nThis route is cached for up to 3600 seconds',
                         ...
                         'parameters': [{'$ref': '#/parameters/datasource'},
                                        ...
                                        {'description': 'A valid war ID',
                                         ...
                                         'minimum': 1, 'name': 'war_id',
                                         'required': True, 'type': 'integer'}],
                         'responses': {'200': {'description': 'A list of killmail IDs and hashes',
                                               'examples': {'application/json': [{'killmail_hash': '8eef5e8fb6b88fe3407c489df33822b2e3b57a5e',
                                                                                  'killmail_id': 2},
                                                                                 {'killmail_hash': 'b41ccb498ece33d64019f64c0db392aa3aa701fb',
                                                                                  'killmail_id': 1}]},
                                               'headers': {'Cache-Control': {'description': 'The caching mechanism used',
                                                                             'type': 'string'},
                                                           'ETag': {'description': 'RFC7232 compliant entity tag',
                                                                    'type': 'string'},
                                                           'Expires': {'description': 'RFC7231 formatted datetime string',
                                                                       'type': 'string'},
                                                           'Last-Modified': {'description': 'RFC7231 formatted datetime string',
                                                                             'type': 'string'},
                                                           'X-Pages': {'default': 1,
                                                                       'description': 'Maximum page number',
                                                                       'format': 'int32',
                                                                       'type': 'integer'}},
                                               'schema': {'description': '200 ok array',
                                                          'items': {'description': '200 ok object',
                                                                    'properties': {'killmail_hash': {'description': 'A hash of this killmail',
                                                                                                     'title': 'get_wars_war_id_killmails_killmail_hash',
                                                                                                     'type': 'string'},
                                                                                   'killmail_id': {'description': 'ID of this killmail',
                                                                                                   'format': 'int32',
                                                                                                   'title': 'get_wars_war_id_killmails_killmail_id',
                                                                                                   'type': 'integer'}},
                                                                    'required': ['killmail_id', 'killmail_hash'],
                                       ...
                                       '400': {'description': 'Bad request',
                                               'examples': {'application/json': {'error': 'Bad request message'}},
                                               'schema': {'$ref': '#/definitions/bad_request'}},
                                       '420': {'description': 'Error limited',
                                               'examples': {'application/json': {'error': 'Error limited message'}},
                                               'schema': {'$ref': '#/definitions/error_limited'}},
                                       '422': {'description': 'War not found',
                                               'examples': {'application/json': {'error': 'Unprocessable entity message'}},
                                               'schema': {'description': 'Unprocessable entity',
                                                          'properties': {'error': {'description': 'Unprocessable entity message',
                                                                                   'title': 'get_wars_war_id_killmails_422_unprocessable_entity',
                                                                                   'type': 'string'}},
                                                          'title': 'get_wars_war_id_killmails_unprocessable_entity',
                                                          'type': 'object'}},
                                       '500': {'description': 'Internal server error',
                                               'examples': {'application/json': {'error': 'Internal server error message'}},
                                               'schema': {'$ref': '#/definitions/internal_server_error'}},
                                       '503': {'description': 'Service unavailable',
                                               'examples': {'application/json': {'error': 'Service unavailable message'}},
                                               'schema': {'$ref': '#/definitions/service_unavailable'}},
                                       '504': {'description': 'Gateway timeout',
                                               'examples': {'application/json': {'error': 'Gateway timeout message'}},
                                               'schema': {'$ref': '#/definitions/gateway_timeout'}}},
                         'summary': 'List kills for a war', 'tags': ['Wars'],
                         'x-alternate-versions': ['dev', 'legacy', 'v1'],
